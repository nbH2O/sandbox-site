...
<div {!! $attributes->merge(['class' => 'relative overflow-visible']) !!} x-data="{ open: false }">
    ...
    <div x-on:click="{{ $innerClick ? 'open = !open' : null }}" x-on:click.away="open = false" class="{{ $alignVals[$align] }} z-[1000] {{ $yAdjust ? 'py-0.5' : null }} min-w-44 bg-[#222226] dark:bg-header rounded-[1.25rem] absolute top-[calc(100%_+_0.84rem)]" x-show="open">
        {{ $slot }}
    </div>
</div>